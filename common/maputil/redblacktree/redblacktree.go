...
func defaultBytesComparator(bytes1, bytes2 []byte) int {
	min := len(bytes2)
	...
	for i := 0; i < min && diff == 0; i++ {
		diff = int(bytes1[i]) - int(bytes2[i])
	}
	if diff == 0 {
		diff = len(bytes1) - len(bytes2)
	}
	if diff < 0 {
		return -1
	}
...
func NewTree(comparator... Comparator) *Tree {
	if len(comparator) > 0 {
		return &Tree{Comparator: comparator[0]}
	} else {
		return &Tree{Comparator: defaultBytesComparator}
	}
}
